// -*- C++ -*-
// $RCSfile: cskeleton.C,v $
// $Revision: 1.137 $
// $Author: langer $
// $Date: 2014/12/31 01:32:24 $
  ...
  const
{
  ...
  unsigned int n = ms.nCategories();
  // std::cerr << "categoryAreas: nCategories=" << n << std::endl;
  DoubleVec *result = new DoubleVec(n);
  ...
  for(std::vector<PixelSetBoundary>::size_type i=0; i<bdys.size(); i++) {
    ...
    // std::cerr << "categoryAreas: Examining a new category, " << i
	...
	for(unsigned int k=0; k<loopsize; ++k) {
	  ...
	  // 	    << " interior=" << pbs_start_inside << std::endl
	  // 	    << "               pbs_end  = " << pbs_end
	  ...
	  // there's an intersection.  Find it.
		...
		accum_area += loop.coord(k) % pbi.location;
	      }
	      else {
		// std::cerr << "categoryAreas: out->in, dA="
		// 	  << pbi.location % loop.next_coord(k) << std::endl;
		accum_area += pbi.location % loop.next_coord(k);
	      }
    ...
    // Debugging
    // std::cerr << "categoryArea: coordisecs" << std::endl;
...
// #endif	// DEBUG
	...
	if(ninter > 1) {
	  ...
	  if(entries != exits) {
	    // TODO: can entries-exits ever be something other than
	    // -1, 0, or 1?  No, because the directions and number of
	    // pixel boundary segments at a point are limited.  This
	    // code is actaully more general than it needs to be.
    ...
    int icount = 0;
    for(n=0; n<nn; n++) {	// loop over element edges
	...
	// you're not outside, do nothing.
      ...
      // needs to play the same game later on.
      if (accum_area < 0.0) {
